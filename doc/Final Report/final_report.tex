...
\begin{align*}
    c_{r} &= \alpha*r_{1} + \beta*r_{2} + \gamma*r_{3}					\\
    c_{b} &= \alpha*b_{1} + \beta*b_{2} + \gamma*b_{3}					\\
    c_{g} &= \alpha*g_{1} + \beta*g_{2} + \gamma*g_{3} \\
\end{align*}
...
Content & \verb!7'b0! & \verb!y! & \verb!6'b0! & \verb!x! & \verb!8'b0! & \verb!red!  \\ \hline
Bits & [47:46] & [45:41] & [40:39] & [38:34] & [33:32] & [31:0] \\ \hline
Content & \verb!2'b0! & \verb!green! & \verb!2'b0! & \verb!blue! & \verb!2'b0! & \verb!z! \\ \hline
\end{tabular}
\end{center}
...
It soon became apparent we could not do everything. The decision to conform to the Xilinx tool chain stemmed partially from the convenience of provided Xilinx cores and generated cores from CoreGEN.  The framework provided by the Xilinx EDK had some advantages.  It has a way to run C code a soft processor, the Microblaze.  It provides the interconnecting bus structure, PLB.  The reference design also contains the Multi-Port Memory Controller\cite{mpmcds} (MPMC) which provides an accessible interface for the DD2 SDRAM.  
...
I was mainly in charge of any software that ran on the Microblaze processor.  When we were considering a web-server front-end for our accelerator, I was working with the lwip library on the SDK trying to get the web server running.  This involved learning the SDK interface along with the EDK interface; initially this was annoying as the design suite was in a transitionaly phase separating functionality between the two.  In fact, the EDK still provided some software configuration deprecated functions meant for the SDK, so there was an overlap of functionality.
...
\begin{itemize}
\item Professor Bill Nace and course staff for running this semester's capstone course, a graduation requirement
\item Professor Ken Mai for an initial meeting and steering our project from the beginning
\item Google Docs for making collaboration simple, but kinda ugly
\item Tumblr for host our blog and recorded missteps
\end{itemize}
...
\bibitem{TFTds}
Xilinx, Inc. 
\emph{Advanced XPS Thin Film Transistor
(TFT) Controller (v2.01a)}.
\underline{Product Specification DS695}.
May 03, 2010.

\bibitem{plbmds}
Xilinx, Inc. 
\emph{PLBV46 Master Single
(v1.01a))}.
\underline{Product Specification DS536}.
May 14, 2008.

\bibitem{edkcttds}
Xilinx, Inc. 
\emph{EDK Concepts, Tools,
and Techniques v11.1}.
\underline{UG683}.
December 02, 2009.

\bibitem{mpmcds}
Xilinx, Inc. 
\emph{Multi-Port Memory Controller
(MPMC) (v6.01.a)}.
\underline{Product Specification DS643}.
July 23, 2010.
